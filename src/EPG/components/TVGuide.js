...
export default class TVGuide extends Component {
    ...
    constructor(props) {
        ...
        fetch('http://demo6835780.mockable.io/')
        .then((res) => res.json())
        .then((data) => {
           console.log(data);
           this.epgData.setChannels(data);
           this.setState({
            loading: false,
          });
        ...
        for (let pos = firstPos; pos <= lastPos; pos++) {
            ...
            for (let event of epgEvents) {
                ...
                if (this.isEventVisible(event.start, event.end)) {
                    this.drawEvent(canvas, pos, epgEvent, drawingRect);
                    foundFirst = true;
                } else if (foundFirst) {
                    break;
                }
    ...
    drawEvent(canvas, channelPosition, event, drawingRect) {
        ...
        this.setEventDrawingRectangle(channelPosition, event.start, event.end, drawingRect);
        // Background
        //mPaint.setColor(event.isCurrent() ? mEventLayoutBackgroundCurrent : mEventLayoutBackground);
        canvas.fillStyle = this.isCurrent(event.start, event.end) ? this.mEventLayoutBackgroundCurrent : this.mEventLayoutBackground;
        if (channelPosition == this.getFocusedChannelPosition()) {
            ...
            } else if (this.isCurrent(event.start, event.end)) {
        ...
        switch (keyCode) {
            case 39 :
                ...
                if (programPosition != -1 && programPosition < this.epgData.getEventCount(this.getFocusedChannelPosition())) {
                    ...
                    if (this.focusedEvent) {
                        ...
                        dx =  parseInt((this.focusedEvent.end - this.focusedEvent.start) / this.mMillisPerPixel);
                    }
                }
            ...
            case 37 :
                ...
                if (programPosition != -1 && programPosition > -1) {
                    ...
                    if (this.focusedEvent) {
                        this.focusedEventPosition = programPosition;
                        dx = (-1) * parseInt((this.focusedEvent.end - this.focusedEvent.start) / this.mMillisPerPixel);
                    }
                }
                this.scrollX = this.getScrollX(false) + dx;
                break;
